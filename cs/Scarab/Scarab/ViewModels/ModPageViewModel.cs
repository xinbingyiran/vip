...
public partial class ModPageViewModel : ViewModelBase
{
    ...
    public ReactiveCommand<Unit, Unit> CacheAll { get; }
    ...
    public bool CanCacheAll => _items.Count > 0;
    ...
    public ModPageViewModel(ISettings settings, IModDatabase db, IInstaller inst, IModSource mods, ILogger logger, HttpClient hc)
    {
        ...
        CacheAll = ReactiveCommand.CreateFromTask(CacheAllAsync);
    ...
    {
        _logger.LogInformation("Caching all mods!");
        ...
        try
        {
            ...
            await Parallel.ForEachAsync(items, async (item, _) =>
            {
                try
                {
                    await item.OnCache(_installer, progress => { });
                    Dispatcher.UIThread.Invoke(() =>
                    {
                        Progress = Interlocked.Increment(ref current) * 100.0 / total;
                        CompletedAction?.Invoke(ModAction.Cache, item);
                    });
                }
                catch (Exception e)
                {
                    _logger.LogError(e, "Error when Caching {Mod}!", item.Name);
                    Dispatcher.UIThread.Invoke(() =>
                    {
                        ExceptionRaised?.Invoke(ModAction.Cache, e, item);
                    });
                }
            });
        }
    ...
    [UsedImplicitly]
    private Task OnCacheAsync(ModItem item) => InternalUpdateInstallAsync(ModAction.Cache, item, item.OnCache);
