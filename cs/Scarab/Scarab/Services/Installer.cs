...
public class Installer : IInstaller
{
    ...
    public async Task Cache(ModItem mod, Action<ModProgressArgs> setProgress)
    {
        await _semaphore.WaitAsync();
    ...
    private async Task _Cache(ModItem mod, Action<ModProgressArgs> setProgress)
    {
        string dir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Mods");
        if (!Directory.Exists(dir))
        {
            Directory.CreateDirectory(dir);
        }
        string filename = new FileInfo(new Uri(mod.Link).LocalPath).Name;
        string fullFile = Path.Combine(dir, filename);
        ArraySegment<byte> data;
        if (File.Exists(fullFile))
        {
            data = await File.ReadAllBytesAsync(fullFile);
        }
        else
        {
            data = await DownloadFile(mod.Link, DownloadProgressed);
            await File.WriteAllBytesAsync(fullFile, data.ToArray());
        }
    ...
    )
    {
        ...
        string dir = Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "Api");
        if (!Directory.Exists(dir))
        {
            Directory.CreateDirectory(dir);
        }
        string filename = new FileInfo(new Uri(api_url).LocalPath).Name;
        string fullFile = Path.Combine(dir, filename);
        ArraySegment<byte> data;
        if (File.Exists(fullFile))
        {
        ...
        {
            data = await DownloadFile(api_url, delegate
            {
            });
            await File.WriteAllBytesAsync(fullFile, data.ToArray());
        }
    ...
    private async Task _Install(ModItem mod, Action<DownloadProgressArgs> setProgress, bool enable)
    {
        ...
        {
            data = await DownloadFile(mod.Link, setProgress);
            await File.WriteAllBytesAsync(fullFile, data.ToArray());
        }
    ...
    private async Task<ArraySegment<byte>> DownloadFile(string uri, Action<DownloadProgressArgs> setProgress)
    {
        return (await _hc.DownloadBytesWithProgressAsync(new Uri(uri), new Progress<DownloadProgressArgs>(setProgress))).Item1;
    }
