...
internal class NetServer : INetServer {
    ...
    public void Start(int port) {
        ...
        Array.ForEach(_udpSockets, s => {
            ...
            s.IOControl((int) SIO_UDP_CONNRESET, [0], null);
            s.Bind(new IPEndPoint(s.AddressFamily is AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, port));
        });
        ...
        Array.ForEach(_udpSockets, s => new Thread(() => ReceiveData(s, _taskTokenSource.Token)).Start());
    }
    ...
    private void ReceiveData(Socket s, CancellationToken token) {
        ...
        EndPoint endPoint = new IPEndPoint(s.AddressFamily is AddressFamily.InterNetwork ? IPAddress.Any : IPAddress.IPv6Any, 0);
        ...
        while (!token.IsCancellationRequested) {
            ...
            while (_receivedQueue.TryDequeue(out var receivedData)) {
                ...
                if (!_clients.TryGetValue(endPoint, out var client)) {
                    ...
                    client = CreateNewClient(receivedData);
    ...
    /// </summary>
    /// <param name="data">The ReceivedData of the new client.</param>
    /// <returns>A new net server client instance.</returns>
    private NetServerClient CreateNewClient(ReceivedData data) {
        var netServerClient = new NetServerClient(data.Socket, data.EndPoint);
        netServerClient.UpdateManager.OnTimeout += () => HandleClientTimeout(netServerClient);
        netServerClient.UpdateManager.StartUpdates();

        _clients.TryAdd(data.EndPoint, netServerClient);
    ...
    public void Stop() {
        ...
        Array.ForEach(_udpSockets, s => s.Close());
...
internal class ReceivedData {
    ...
    /// <summary>
    /// The Socket from which we received the data.
    /// </summary>
